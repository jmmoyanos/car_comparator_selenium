...
def get_ad_data(ad_link, sleep_time ,save_to_csv, save_to_pickle):
    ...
    list_all_data_ad = list_data_ad + list_ad_price + p

    data_we_want_to_keep = ['First Registration','Mileage', 'Fuel','Number of Seats','Cubic Capacity','Colour','Gearbox', 'Consume', 'IVA', 'Price', 'model_type', 'location']
    ...
    if save_to_csv:
        ...
        if not os.path.exists('data/es/make_model_ads_data'):
            os.makedirs('data/es/make_model_ads_data')

        df.to_csv(str('data/es/make_model_ads_data/links_on_one_page_df' + datetime_string + '.csv'), index = False)

    if save_to_pickle:
        if not os.path.exists('data/es/make_model_ads_data'):
            os.makedirs('data/es/make_model_ads_data')

        df.to_pickle('data/es/make_model_ads_data/links_on_one_page_df' + datetime_string + "pkl")
    return df
...
def concatenate_dfs(indir, save_to_csv = True, save_to_pickle = True):
    ...
    if save_to_csv:
        output_file.to_csv("data/es/make_model_ads_concatinated.csv", index=False)

    if save_to_pickle:
        output_file.to_pickle("data/es/make_model_ads_concatinated.pkl")
...
if __name__ == '__main__' :
    ...
    make_model_ads_data = pd.read_csv("./data/es/make_model_ads_links_concatinated.csv")
    ...
    individual_ads_data = concatenate_dfs("./data/es/make_model_ads_data/",  True, False)

    ads_df = merge_make_model_keep_latest(data = individual_ads_data)
    ads_df_clean = clean_data(data = ads_df)


    if not os.path.exists('data/es/final_data'):
            os.makedirs('data/es/final_data')

    now = datetime.now() 
    datetime_string = str(now.strftime("%Y%m%d_%H%M%S"))
    ads_df_clean['audit_date'] = datetime_string
    ads_df_clean.to_csv(f'./data/es/final_data/scrap_mobilede_{datetime_string}.csv', index=False)