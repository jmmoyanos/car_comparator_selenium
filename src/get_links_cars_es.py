...
def scrape_links_for_one_make_model(make_model_dat,sleep, save_to_csv):
    ...
    if save_to_csv:
            ...
            if not os.path.exists('data/es/make_model_ads_links'):
                os.makedirs('data/es/make_model_ads_links')

            links_on_one_page_df.to_csv(str('./data/es/make_model_ads_links/links_on_one_page_df' + datetime_string + '.csv'), index = False)
...
def concatenate_dfs(indir, save_to_csv = True, save_to_pickle = True):
    ...
    if save_to_csv:
        output_file.to_csv("data/es/make_model_ads_links_concatinated.csv", index=False)

    if save_to_pickle:
        output_file.to_pickle("data/es/make_model_ads_links_concatinated.pkl")
...
if __name__ == "__main__":
    ...
    make_model_dat = pd.read_csv('./data/es/make_and_model_links.csv')
        
    multi_data = multiple_link_on_multiple_pages_data(make_model_dat,
                                                      1, 
                                                      True)

    make_model_ads_data = concatenate_dfs(indir= "data/es/make_model_ads_links/", save_to_csv = True, save_to_pickle = False)
